...
function MetricCard({ metricKey, data, isActive, onClick }) {
  ...
  return (
    ...
    >
      ...
      {/* Value */}
      <div className="mb-1">
        <span className="stat-value text-xl" style={{ color: isActive ? def.color : undefined }}>
      ...
      {/* Active indicator */}
      {isActive && (
        <div className="mt-3 h-0.5 rounded-full bg-gradient-to-r from-ledger to-transparent" />
      )}
    </button>
  );
}
...
export default function MetricsGrid() {
  ...
  const availableMetrics = Object.entries(metrics).filter(([, data]) => data?.latest);

  if (availableMetrics.length === 0) {
    return (
      <div className="glass-card p-8 text-center">
        <p className="text-ink-200 text-sm">No financial data available for this company.</p>
      </div>
    );
  }
